...
# Default params
inp_dir = _config.OUT_PLACE + f'e2_fullgts_trajectory/'
...
##
def merge_regimes():
  df = pd.read_csv(inp_dir + f'pv_fullgts_trajectory.csv', index_col = 0)
  ...
  for pace_idx in pace_regimes:
    ...
    merged_df.to_csv(out_dir + f'mel_fullgts_trajectory_p{pace_idx}.csv')

    pv_df = merged_df.pivot(index = 'Full genotype', columns = 'Sample', values = 'Frequency')
    pv_df.to_csv(out_dir + f'pv_fullgts_trajectory_p{pace_idx}.csv')
