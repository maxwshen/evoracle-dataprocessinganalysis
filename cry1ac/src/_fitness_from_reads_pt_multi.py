...
class MarginalFitnessModel(nn.Module):
  ...
  def forward(self):
    ...
    for t in range(self.num_timepoints - 1):
      ...
      # Simulate natural selection recurrence for time_step
      # Output: pred_p1
      input_ps = p0
...
def load_data(dataset_nm):
  data_place = '/ahg/regevdata/projects/CRISPR-libraries/prj2/evolution/badran/out/'
  if hparams['datasource'] == 'pacbio':
    data_dir = f'{data_place}/pb_e2_dataset_multi/'
  elif hparams['datasource'] == 'illumina_single':
    data_dir = f'{data_place}/ill_d2_major_singles/'
  elif hparams['datasource'] == 'illumina_full':
    data_dir = f'{data_place}/ill_e2_fullmuts_dataset/'
  elif hparams['datasource'] == 'pb_interpolate':
    data_dir = f'{data_place}/pb_e3_interpolate_multi/'  
...
def run_test():
  ...
  #   custom_hyperparams = 'dataset:simple_smart_rl_1',
  # )

  package = run_inference(
    custom_hyperparams = 'dataset:intsimple_smart_rl_100_noise_0_star_82+datasource:pb_interpolate',
  )
  return
...
if __name__ == '__main__':
  if len(sys.argv) == 2:
    assert sys.argv[1] == 'test', 'Must run test'
    run_test()
  elif len(sys.argv) == 3:
    assert sys.argv[1] == 'gen_qsubs', 'Must gen qsubs'
    gen_qsubs(modelexp_nm = sys.argv[2])
  elif len(sys.argv) == 4:
    run_inference(
      modelexp_nm = sys.argv[1],
      exp_nm = sys.argv[2],
      custom_hyperparams = sys.argv[3],
    )
  else:
    gen_qsubs()
